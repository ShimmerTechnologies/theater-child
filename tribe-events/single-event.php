...
<div id="tribe-events-content" class="tribe-events-single vevent hentry">
	<div id="post-<?php the_ID(); ?>" <?php post_class('cmsmasters_single_event'); ?>>
		...
		<?php	
				...
				$ticket = tribe_get_event_website_url();
				$ticketTypeVal = intval($ticket);

					//if ticket is an integer it's value is greater than 0 after intval() - for performance ID use
					if( $ticketTypeVal > 0 && $ticket != "" ) {

						echo '<div id="cmsmasters_button_8x097x19tk" class="button_wrap"><a href="https://app.arts-people.com/index.php?performance='. $ticket .'" class="cmsmasters_button" ><span>Buy tickets</span></a></div>';

					} else if ($ticket != "") {

						echo '<div id="cmsmasters_button_8x097x19tk" class="button_wrap"><a href="'. $ticket .'" class="cmsmasters_button" ><span>Buy tickets</span></a></div>';
	...
	if ($published_events > 1) {
		echo '<aside id="tribe-events-sub-nav" class="post_nav cmsmasters_single_tribe_nav">';
			...
			tribe_the_prev_event_link('<span class="post_nav_sub">' . esc_html__('Previous Link', 'theater') . '</span>%title%<span class="cmsmasters_prev_arrow"><span>');
			
			tribe_the_next_event_link('<span class="post_nav_sub">' . esc_html__('Next Link', 'theater') . '</span>%title%<span class="cmsmasters_next_arrow"><span>');
